{
  ...
  plot2 = ggplot( data=weatherData ) +
    geom_histogram( mapping=aes(x=avgTemp),
                    color="black") +  
    theme_bw() +
    facet_grid( rows=vars(season) ) + # vars() checks the data frame; needs to use the column
    labs(title = "Temperature (\u00B0F)",
         subtitle = "Lansing, Michigan: 2016",
         x = "Temperature (\u00B0F)");     
  plot(plot2);
  ...
  plot4 = plot3 + # take components from plot3
    facet_grid( cols=vars(seasonOrdered)); #rewrite the facet component
  plot(plot4);
